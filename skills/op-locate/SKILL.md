...
并提示人工 review 后回写 `knowledge/precision_known_issues.md`（不自动回写）。

## 工具与知识

- **工具库**：`op-locate-agent/lib/`（config_loader, path_resolver, hook_manager, tensor_compare）
- **知识库**：`op-locate-agent/knowledge/`（arch_index, vllm_forward_paths, precision_known_issues, hook_pitfalls, platforms/hygon_dcu）
...
**这只是结构性事实，不预设是否有问题**。可读 `knowledge/precision_known_issues.md` 的
...
参考脚本：`examples/compare_layers.py`（逐层/逐算子中间值对比，`--layers` 控制粒度、`--op` 限定算子类）。

> **必读**：`knowledge/hook_pitfalls.md` 陷阱 1-3，否则会得到假误差。

### 阶段 5：细粒度算子定位

定位到发散层后，做层内拆解——**按模型类型分支**：

**MoE 模型** (`profile.is_moe=True`)：按 `knowledge/vllm_forward_paths.md` 的 MoE 拆解树细化：
1. `--op router --layers N`：抓 router_logits 对比 HF。若发散，根因在 gate。
2. 若 router 对齐：`--op mlp --layers N` 抓 mlp_out。若发散，根因在专家 FFN。
3. 若 mlp 也对齐：`--op attn --layers N` 排查 attention。

**Dense 模型**：直接在发散层做算子 drill-down：
```bash
--op attn --layers N          # attention 输出
--op mlp --layers N           # MLP/FFN 输出
--op <norm_attr> --layers N   # RMSNorm / LayerNorm 输出
```

`--op` 接受任意属性名——从 probe 结果取或直接用 `attn/mlp` 等属性路径。

> **必读**：`knowledge/hook_pitfalls.md` 陷阱 4（重算 vs 实际）、陷阱 5（topk 排序错位，MoE 专用）。

### 阶段 6：单算子对比验证

锁定嫌疑算子后，做照实验：

- **通用方法**：同一输入对比 vLLM 的融合 kernel vs 纯 PyTorch 回退路径。
  - 若嫌疑是 fused FFN（MoE）：`forward_cuda` vs `forward_native`
  - 若嫌疑是 flash_attn：设 `VLLM_ATTENTION_BACKEND=TORCH_SDPA` 回退对比
  - 若嫌疑是 fused_gate（MoE）：`ops.moe_fused_gate` vs Python `grouped_topk`
  - 均用 `--env` 设对应环境变量做端到端对照

- **MoE 专属**：用 `examples/compare_layers.py --op router` + `--env` 做有/无 fused_gate 对照。

参考脚本：`examples/compare_layers.py --op`；`lib.compare_topk`（MoE 专属）。
...
python3 examples/generate_report.py \
    --model <模型路径> \
    --compare-dir /tmp/compare_layers_xxx \   # --op 细化的落盘；不传则取 /tmp 最新
...
**多轮细化聚合**：粗粒度逐层 + `--op router` + `--op mlp` 各跑一次 compare_layers，用 `--compare-dirs`（可多次）把多个落盘传给 generate_report，被证伪的 router 假设才会进报告的"被证伪假设"表。

最后**提示人工 review** 后回写 `knowledge/precision_known_issues.md`。**不自动回写**。
